...
export class AuthController {
    ...
    @Post('login')
    async login(@Body() body: { account_id: string; password: string }): Promise<{ message: string; token: string }> {
        const token = await this.authService.login(body.account_id, body.password);
